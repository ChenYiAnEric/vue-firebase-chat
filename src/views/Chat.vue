    ...
    <!-- <div class="chat-room" :class="{ 'pa-4': RWDTab !== 'xs' }">
      <v-col v-if="isLogin">
        <v-card max-height="70vh" style="overflow: auto">
          <v-card-text>
            <v-flex
              v-for="(item, key) in MessageSort"
              :key="key"
              :class="isMy(item.Author.Email, false)"
            >
              <p v-if="!isMy(item.Author.Email, true)" class="ma-0 ml-2">
  ...
  computed: {
    ...
    MessageSort() {
      ...
      const sort = message
        .sort(function(a, b) {
          return (
            new Date(b.CreateTime.toDate()) - new Date(a.CreateTime.toDate())
          );
        })
        .reverse();
      return sort;
